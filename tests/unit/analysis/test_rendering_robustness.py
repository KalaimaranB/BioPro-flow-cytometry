...
from karcytics_plugins.flow_cytometry.analysis.rendering import compute_pseudocolor_points
...
def test_pseudocolor_rank_normalization():
    ...
    # compute_pseudocolor_points returns points sorted in ASCENDING order of density.
    # Therefore, the last N1 points will contain the dense cluster cores, and the
    # first N2 points will contain the sparse fringes.
    c_sparse = c[:N2]  # Lowest density points
    c_dense = c[-N2:]  # Highest density points

    assert np.max(c) == pytest.approx(1.0, abs=0.05)
    assert np.mean(c_dense) > np.mean(c_sparse)
    assert np.max(c_sparse) > 0.0  # Should not be completely zeroed
