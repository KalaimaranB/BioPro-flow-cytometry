...
def test_rendering_constants_have_not_drifted():
    ...
    assert constants.DEFAULT_NBINS_MAX == 8192
    assert constants.NBINS_SCALING_FACTOR == 1.5

    assert constants.SIGMA_MIN == 0.1
    assert constants.SIGMA_SCALING_FACTOR == 2.2

    assert constants.DENSITY_THRESHOLD_MIN == 0.05
