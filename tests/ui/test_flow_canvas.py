...
from ui.graph.flow_canvas import (
    DisplayMode,
    FlowCanvas,
    GateDrawingMode,
)
...
class TestFlowCanvasEventState:
    ...
    def test_track_mouse_position(self):
        ...
        canvas._fsm._drag_start = (150, 250)

        assert canvas._fsm._drag_start == (150, 250)
    ...
    def test_polygon_vertex_tracking(self):
        ...
        for x, y in vertices:
            canvas._fsm._polygon_vertices.append((x, y))

        assert len(canvas._fsm._polygon_vertices) == 4
        assert canvas._fsm._polygon_vertices == vertices

        # Clear vertices
        canvas._fsm._polygon_vertices.clear()
        assert len(canvas._fsm._polygon_vertices) == 0
