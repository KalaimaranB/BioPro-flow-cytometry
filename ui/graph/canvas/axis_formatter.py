...
class AxisFormatter:
    ...
    def apply_formatting(self):
        ...
        if x_scale.transform_type != TransformType.LINEAR:
            raw_ticks, labels = self._build_bio_ticks(x_scale, x_scale.transform_type == TransformType.BIEXPONENTIAL)
        ...
        if display_mode not in (DisplayMode.HISTOGRAM, DisplayMode.CDF):
            if y_scale.transform_type != TransformType.LINEAR:
                raw_ticks, labels = self._build_bio_ticks(y_scale, y_scale.transform_type == TransformType.BIEXPONENTIAL)
    ...
    def _add_linear_region_shading(self, ax, mapper, axis: str) -> None:
        ...
        if axis == "x":
            ...
            ax.axvspan(disp_bounds[0], disp_bounds[1], color="#000000", alpha=0.03, zorder=0, linewidth=0)
        else:
            disp_bounds = mapper.transform_y(raw_bounds)
            ax.axhspan(disp_bounds[0], disp_bounds[1], color="#000000", alpha=0.03, zorder=0, linewidth=0)
