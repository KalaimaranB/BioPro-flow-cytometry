...
from PyQt6.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QTabWidget, QScrollArea, QGridLayout,
...
class ClusterResultsPanel(QWidget):
    ...
    def __init__(self, results: dict[str, Any], state=None, gate_coordinator=None, parent: QWidget | None = None) -> None:
    ...
    def _create_plot(self, embedding: np.ndarray, color_data: np.ndarray, title: str, cmap: str, norm=None, is_discrete=False, min_c=0, max_c=0) -> CopyableCanvas:
        ...
        scatter = ax.scatter(
            ...
            c=color_data, cmap=cmap, norm=norm,
            s=1.0, alpha=0.75, edgecolors="none"
        )
    ...
    def _build_plot_gallery(self) -> None:
        ...
        for i, ch in enumerate(channels):
            ...
            canvas = self._create_plot(
                embedding, intensities[:, i], ch, "viridis"
            )
    ...
    def _build_populations_tab(self) -> None:
        from PyQt6.QtWidgets import QPushButton, QLineEdit, QScrollArea, QWidget, QVBoxLayout, QCheckBox, QLabel
        from biopro_sdk.plugin.components import PrimaryButton, SecondaryButton, BioLineEdit
    ...
    def _create_populations(self) -> None:
        ...
        umap_parent = target_node.add_child(
            gate=SubsetGate(indices=indices),
            name="UMAP Reduction"
        )
            ...
            for cluster_id, (checkbox, name_edit) in self._cluster_ui_elements.items():
                if checkbox.isChecked():
                    ...
                    umap_parent.add_child(
                        gate=SubsetGate(indices=cluster_indices),
                        name=name
                    )
                    created_count += 1
    ...
    def _activate_polygon_drawer(self) -> None:
        ...
        from biopro_sdk.plugin.dialogs import show_info
        show_info(self, "Draw Cluster", "Click on the UMAP plot to draw a polygon.\nDouble-click or press Enter to complete the shape.")
        
        self._poly_selector = PolygonSelector(
            ax, self._on_polygon_drawn,
            useblit=True,
