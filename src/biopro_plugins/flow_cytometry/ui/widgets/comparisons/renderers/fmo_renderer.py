    ...
    def render(self, **kwargs) -> Figure:
        ...
        if len(fmo_values) > 0:
            ax.hist(
                ...
                color="#8b949e",
                linewidth=1.5,
                label=f"{fmo_label} (n={len(fmo_values):,})",
                density=True,
            )
        ...
        if len(sample_values) > 0:
            ax.hist(
                ...
                label=f"{sample_label} (n={len(sample_values):,})",
                density=True,
            )

        # Suggested gate line at FMO 99th percentile
        if show_gate_line and len(fmo_values) > 10:  # noqa: PLR2004
            ...
            ax.axvline(
                ...
                linewidth=1.5,
                linestyle="--",
                label=f"Suggested gate ({gate_pos:.1f})",
            )
