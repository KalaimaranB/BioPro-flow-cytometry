...
Run by `biopro_sdk.plugin.PluginUIDaemon` from this plugin's own `.venv`
...
plugin has ~80+ call sites importing `biopro.core.*`/`biopro.ui.*` directly
(grep for "from biopro.core" / "from biopro.ui" across src/) rather than going
...
# rather than imported as part of the `biopro_plugins` package — nothing else
...
# CRITICAL: same reasoning as biopro_plugins.flow_cytometry.__init__ — must be
...
class _FakeBioProEvent:
    """Fake stand-in for `biopro.core.event_bus.BioProEvent`.

    The real thing is an `Enum`; plugin code only ever does attribute access
    on it (`BioProEvent.MODULE_OPENED`) and passes the result straight into
...
class _ColorsFallback:
    ...
    `sys.modules["biopro.ui.theme"]` self-check, which is what caused the
    infinite recursion `_install_core_shims()` works around by aliasing
    `DynamicColors` instead of the proxy.
    """

    def __getattr__(self, name: str) -> str:
        from biopro_sdk.plugin.theme_fallback import DynamicColors
...
def _install_core_shims(plugin_module: Any, task_scheduler: _LocalTaskScheduler) -> None:
    r"""Stub the `biopro.core.*` modules this plugin imports directly, bypassing
    `PluginContext` entirely, so its code can run unmodified out-of-process.

    Grep for `from biopro\.core` across this repo turns up ~20 call sites
    ...
    - `biopro.core.task_scheduler.task_scheduler` -> the same
    ...
    - `biopro.core.event_bus.event_bus`/`BioProEvent` -> forwards `.emit()`
      ...
      this way).
    - `biopro.core.diagnostics.diagnostics` -> forwards `.report_error()` to
      the Hub as a "diagnostics_error" event instead of the Hub's real
      diagnostics reporter.
    - `biopro.core.tutorial_manager.global_tutorial_manager` -> an inert stub,
      *and* `register_courses` is replaced with a no-op on the plugin module
      ...
      `tutorials/courses.py`, which pulls in `biopro.core.models.tutorial_models`,
    ...
    _stub_module("biopro.core.task_scheduler", task_scheduler=task_scheduler)
    _stub_module("biopro.core.event_bus", event_bus=_EventBusShim(), BioProEvent=_FakeBioProEvent())
    _stub_module("biopro.core.diagnostics", diagnostics=_DiagnosticsShim())
    _stub_module(
        "biopro.core.tutorial_manager",
        global_tutorial_manager=types.SimpleNamespace(courses_by_module={}),
    )

    # biopro.ui.theme: ~60 call sites across this plugin do
    # `from biopro.ui.theme import Colors, Fonts[, theme_manager]` for
    ...
    # `__getattr__` checks `sys.modules["biopro.ui.theme"]` on every access —
    # aliasing that proxy *as* `biopro.ui.theme` itself makes it look itself
    # up forever).
    if "biopro.ui.theme" not in sys.modules:
        import biopro_sdk.plugin.theme_fallback as _theme_fallback
...
class _LocalTaskScheduler:
    ...
    each other — see `biopro/core/task_scheduler.py`. That concern doesn't
    ...
    def submit(self, analyzer: Any, state: Any = None) -> Any:
        from biopro_sdk.plugin import AnalysisRunnable, AnalysisWorker
...
class _EventBusShim:
    ...
    (`emit`/`subscribe`/`unsubscribe`) — `biopro.core.event_bus.event_bus` is
...
def _build_plugin_context(task_scheduler: _LocalTaskScheduler) -> Any:
    ...
    manifest = PluginManifest(
        name="flow_cytometry",
        entry_point="biopro_plugins.flow_cytometry:initialize",
        sdk_version="2.0",
...
def main() -> None:  # noqa: C901
    ...
    from biopro_plugins.flow_cytometry import initialize
