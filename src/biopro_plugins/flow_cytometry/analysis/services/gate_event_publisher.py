...
class GateEventPublisher:
    ...
    @staticmethod
    def publish_gate_created(
        sample_id: str, node_id: str, gate_id: str, name: str, is_split: bool = False
    ) -> None:
    ...
    def publish_gate_deleted(sample_id: str, node_id: str, gate_id: str) -> None:
        CentralEventBus.publish(
            events.GATE_DELETED,
            {"sample_id": sample_id, "node_id": node_id, "gate_id": gate_id},
        )
    ...
    def publish_gate_renamed(sample_id: str, node_id: str, new_name: str) -> None:
        CentralEventBus.publish(
            events.GATE_RENAMED,
            {"sample_id": sample_id, "node_id": node_id, "new_name": new_name},
        )
    ...
    def publish_stats_computed(sample_id: str, node_id: str, stats: dict) -> None:
        CentralEventBus.publish(
            events.STATS_COMPUTED,
            {"sample_id": sample_id, "node_id": node_id, "stats": stats},
        )
