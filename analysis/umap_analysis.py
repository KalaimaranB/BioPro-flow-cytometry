...
from .fcs_io import get_fluorescence_channels, get_channel_marker_label
...
class UmapAnalysis(AnalysisBase):
    ...
    def validate(self, state: Any) -> tuple[bool, str]:
        ...
        if len(sample.fcs_data.events) < 50:
            return False, f"Sample '{sample.display_name}' has too few events ({len(sample.fcs_data.events)}) for UMAP analysis."
    ...
    def run(self, state: Any) -> dict[str, Any]:
        ...
        sample = state.experiment.samples[sample_id]
        ...
        # 5. Fit UMAP
        logger.info(f"UmapAnalysis: Fitting UMAP on shape {X.shape} with n_neighbors={self.n_neighbors}, min_dist={self.min_dist}")
